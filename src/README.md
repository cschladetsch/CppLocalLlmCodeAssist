...
| `LocalCommands.cpp` | `FindRepoRoot`, `ResolveChatFileRoot`, `TryHandleLocalCommand` | The `/pwd`, `/ls`, `/read`, `/write` chat commands `ChatServer` answers itself, plus the repo-root/default-root logic that picks the area they're confined to. Takes the root as a parameter so it's testable against a temp directory. |
